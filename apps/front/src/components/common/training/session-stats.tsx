...
function Delta({
  ...
  return (
    <span
      className={`flex items-center gap-0.5 text-[10px] ${good ? "text-emerald-500" : "text-rose-500"}`}
    >
...
export function SessionStats({ stats }: SessionStatsProps) {
  const pace =
    stats.edges_per_minute != null ? `${stats.edges_per_minute}/min` : "—";
  const pacePrev =
    stats.edges_per_minute_previous != null
      ? `${stats.edges_per_minute_previous}/min`
      : null;
  const paceAvg =
    stats.edges_per_minute_average != null
      ? `${stats.edges_per_minute_average}/min`
      : null;
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <MetricCard
          ...
          previous={
            stats.edges_previous != null ? String(stats.edges_previous) : null
          }
          average={
            stats.edges_average != null ? String(stats.edges_average) : null
          }
          delta={
