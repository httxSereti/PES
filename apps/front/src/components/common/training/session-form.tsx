...
import { Field, FieldDescription, FieldError, FieldGroup, FieldLabel } from "@pes/ui/components/field";
    ...
    goals: z
        .array(
            z.object({
                ...
                value: z.coerce.number({ message: "Must be a number." }).positive("Must be positive."),
            }),
        )
...
export function toSessionFields(values: SessionFormValues): TrainingSessionFields {
    return {
        name: values.name,
        goals: values.goals.map((goal) => ({
            type: goal.type,
            value: goal.type === "duration" ? Math.round(goal.value * 60) : Math.round(goal.value),
        })),
        auto_stop_on_goal: values.auto_stop_on_goal,
    };
}

export function fromSessionFields(fields: Partial<TrainingSessionFields> | undefined): SessionFormValues {
    return {
        name: fields?.name ?? "",
...
export function SessionForm({ initial, submitLabel, onSubmit, error }: SessionFormProps) {
    const form = useForm<
    ...
    return (
        ...
        >
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Select value={field.value} onValueChange={field.onChange}>
                                        <SelectTrigger className="w-48" aria-label="Goal type">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {GOAL_TYPES.map((type) => (
            ...
            <Controller
                ...
                control={form.control}
                render={({ field }) => (
                    <Field className="flex items-center justify-between gap-4 rounded-lg border border-border/60 p-3">
                        <div>
                            <FieldLabel>Auto stop on success</FieldLabel>
                            <FieldDescription>
                                End the session automatically when all goals are reached.
                                Otherwise the Host decides to end or continue.
                            </FieldDescription>
                        </div>
                        <Switch
