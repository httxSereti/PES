...
When a backend change affects the frontend, the contract is the WS `{type, payload}` message — declared in **`apps/app/src/api/ws/schema.py`** (pydantic models, `WS_SCHEMA_VERSION`, sent to clients as `schemaVersion` in the `connected` handshake). After editing the schema, regenerate the TS contract with `pnpm codegen:ws` (writes `apps/front/src/types/websocket.generated.ts` — never edit it by hand) and keep the frontend reducer mapping in `websocketMiddleware.ts` in sync.
