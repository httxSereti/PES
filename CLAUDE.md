...
**Users are persisted** (`users` table: role, custom_permissions JSON, is_active, last_login_at) with magic login tokens in `magic_tokens` (SHA-256 hash only, 7-day expiry, single-use). The `Store` user map is an in-memory **cache**: it is loaded from the DB in the FastAPI lifespan, and every mutation goes through `UserService` (`src/services/users.py`), which writes through to the DB and refreshes live WS permission snapshots. If no active ROOT user exists at startup, one is created; a **fresh ROOT magic token is issued on every boot** (previous unused tokens are revoked) and the link printed to the console — the console is the root-of-trust recovery path.
